...
from BCQLinear import BCQLinear
...
def load_safetensors_state_dict(model, folder, strict=False, verbose=True):
    folder = os.path.abspath(folder)
    index_file = os.path.join(folder, "model.safetensors.index.json")

    if os.path.isfile(index_file):
        print(f"[INFO] Found index file: {index_file}")
        ...
        print(f"[INFO] Loaded {len(loaded_shards)} shards, {len(state_dict)} tensors total.")

    else:

        one_file = os.path.join(folder, "model.safetensors")
        if not os.path.isfile(one_file):
            raise FileNotFoundError(f"[ERROR] No model.safetensors or index.json found in {folder}")

        print(f"[INFO] Loading single safetensor: {one_file}")
        state_dict = load_file(one_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--backend', type=str, default=None, help='quantization backend to use', choices=["bcq", "qtip", None])
