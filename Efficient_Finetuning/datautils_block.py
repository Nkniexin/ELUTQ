...
def get_c4(tokenizer, train_size, val_size, seed, seqlen, test_only):
    ...
    try:
        ...
        traindata = load_dataset(
            'json',
            data_files={'train': '/home/nku509/datasets/c4_dataset/c4-train.00000-of-01024.json.gz'},
            split='train'
        )

        valdata = load_dataset(
            'json',
            data_files={'validation': '/home/nku509/datasets/c4_dataset/c4-validation.00000-of-00008.json.gz'},
            split='validation'
        )
...
def get_redpajama(tokenizer, train_size, val_size, seed, seqlen):
    print("get_redpajama")
    try:
        loacal_dataset = "/home/nku509/.cache/huggingface/datasets/togethercomputer___red_pajama-data-1_t-sample"
...
class BlockTrainDataset(Dataset):
    def __init__(self, size, seqlen, hidden_size, batch_size, dtype, cache_path='./cache/block_training_data', off_load_to_disk=False,disk_data_block_size = 256):
        self.size = size
        self.seqlen = seqlen
        ...
        if size < disk_data_block_size :
            disk_data_block_size = size
        self.disk_data_block_size = disk_data_block_size
        assert size%batch_size == 0

        if self.off_load_to_disk :
            assert size%disk_data_block_size == 0
            assert disk_data_block_size%batch_size == 0
            self.disk_data_start_index = -1
            self.disk_data_end_index = -1
        if self.off_load_to_disk:
            if not os.path.exists(self.cache_path):
                os.makedirs(self.cache_path)
                self._initialize_data_on_disk()
            self.data = torch.zeros((self.disk_data_block_size // self.batch_size,self.batch_size,self.seqlen,self.hidden_size),dtype=self.dtype)
        else:
    ...
    def _initialize_data_on_disk(self):
        for idx in range(self.size // self.disk_data_block_size):
            tensor = torch.zeros((self.disk_data_block_size // self.batch_size, self.batch_size, self.seqlen, self.hidden_size), dtype=self.dtype)
    ...
    def __getitem__(self, idx):
        ...
        if self.off_load_to_disk:
            ...
            if idx < self.disk_data_start_index or idx >= self.disk_data_end_index :

                block_idx = idx // (self.disk_data_block_size // self.batch_size)
                filepath = self._get_file_path(block_idx)
                self.data = torch.load(filepath)

                self.disk_data_start_index = block_idx * (self.disk_data_block_size // self.batch_size)
                self.disk_data_end_index = (block_idx+1) * (self.disk_data_block_size // self.batch_size)

            idx = idx - self.disk_data_start_index
    ...
    def update_data(self, idx, new_data):
        if self.off_load_to_disk:
            if idx < self.disk_data_start_index or idx >= self.disk_data_end_index :
                ...
                self.disk_data_start_index = block_idx * (self.disk_data_block_size // self.batch_size)
                self.disk_data_end_index = (block_idx+1) * (self.disk_data_block_size // self.batch_size)
            real_idx = idx - self.disk_data_start_index
            self.data[real_idx] = new_data

            if idx == self.disk_data_end_index - 1 :
                block_idx = self.disk_data_start_index // (self.disk_data_block_size // self.batch_size)
                filepath = self._get_file_path(block_idx)
                torch.save(self.data.to(self.dtype), filepath)
