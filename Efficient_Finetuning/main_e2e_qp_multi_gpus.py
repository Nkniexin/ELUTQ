...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    num_train_epochs: float = field(default=3.0, metadata={"help": 'The number of epochs to train for'})
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    model, tokenizer = load_quantized_model_multi_devices(args.quant_model_path,args.wbits, args.group_size, args.model_type)
    ...
    if isinstance(tokenizer, LlamaTokenizer):
        ...
        pad_id = None
        if getattr(model.config, "pad_token_id", None) not in [None, -1]:
